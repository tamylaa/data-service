#!/usr/bin/env node
/**
 * Test User Operations in Data Service
 */
...
async function testUserOperations() {
  try {
    ...
    const email = await new Promise((resolve) => {
      rl.question('Enter email to test: ', resolve);
    });
    ...
    console.log(`\n📧 Testing with email: ${email}, name: ${name}\n`);
    
    // 1. Test creating a user
    console.log('🔄 Step 1: Creating/Finding User...');
    const createUserResponse = await fetch(`${DATA_SERVICE_URL}/api/users`, {
      method: 'POST',
      headers: {
    ...
    if (userId) {
      ...
      const getUserResponse = await fetch(`${DATA_SERVICE_URL}/api/users/${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Origin': 'https://www.tamyla.com'
      ...
      const updateResponse = await fetch(`${DATA_SERVICE_URL}/api/users/${userId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Origin': 'https://www.tamyla.com'
      ...
      const verifyResponse = await fetch(`${DATA_SERVICE_URL}/api/users/${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Origin': 'https://www.tamyla.com'
    ...
    const findResponse = await fetch(`${DATA_SERVICE_URL}/api/users/email/${encodeURIComponent(email)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Origin': 'https://www.tamyla.com'
